...
const SpecialityMenu = () => {
  return (
    <div id='speciality' className='flex flex-col items-center gap-4 py-16 text-gray-800'>
        ...
        <div className='flex sm:justify-center gap-14 pt-5 w-full overflow-scroll'>
            {specialityData.map((item, index) => {
                return (
                    <div className=''>
                        <Link onClick={()=>scrollTo(0,0)} className='flex flex-col items-center text-xs cursor-pointer shrink-0 hover:-translate-y-5 transition-all duration-500 ' key={index} to={`/doctors/${item.speciality}`}>
